...
CREATE_COLLAGE = True
COLLAGE_PATH = 'images/collage'
COLLAGE_NAME_TEMPLATE = 'DSC_{0}.jpg'
COLLAGE_IMG_PASTE = 'themes/Polaroid.png'

## Change to what you like to print; IMAGES_PATH, THUMBNAIL_PATH, COLLAGE_PATH
PRINT_PATH = COLLAGE_PATH
...
def create_collage(taken_photo_filename):
	if not os.path.exists(COLLAGE_PATH):
		os.makedirs(COLLAGE_PATH, 0777)

	collage_filename = os.path.join(COLLAGE_PATH, COLLAGE_NAME_TEMPLATE.format(format(get_next_image_number(IMAGES_PATH) - 1, '04d')))

	collage_img = Image.open(taken_photo_filename)
	paste = Image.open(COLLAGE_IMG_PASTE)
	collage_img.paste(paste, (0, 0), paste)

	collage_img.save(collage_filename, "JPEG")
